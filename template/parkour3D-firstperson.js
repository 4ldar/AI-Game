...
const LEVEL_DATA = [
    ...
    { p: [10, 2, 0], s: [2, 1, 2] },
    { p: [13, 4, 3], s: [2, 1, 2] },
    { p: [13, 5.5, 8], s: [2, 1, 2], isFinish: true },
];
const FALL_LIMIT = -20;
...
function buildLevel() {
    ...
    LEVEL_DATA.forEach(data => {
        ...
        const mesh = new THREE.Mesh(new THREE.BoxGeometry(sx, sy, sz), new THREE.MeshStandardMaterial({ color: data.isFinish ? 0xffd700 : 0x8b4513 }));
...
function updatePlayer(deltaTime) {
    ...
    forward.y = 0;
    forward.normalize();
    const right = new THREE.Vector3().crossVectors(player.up, forward).normalize();
    
    let moveVel = new THREE.Vector3();
    if (keys['KeyW']) moveVel.add(forward);
    if (keys['KeyS']) moveVel.sub(forward);
    if (keys['KeyA']) moveVel.sub(right);
    if (keys['KeyD']) moveVel.add(right);
...
function checkGameState() {
    if (playerBody.position.y < FALL_LIMIT) {
        handleEndGame(false);
    }
    platforms.forEach(p => {
